...
import JsonApi.Base.Guide as Guide exposing
  ( Guide, setDoc, updateIdr, updateDoc
  )
...
setLocality field isLocal g = 
  Core.setRelationLocality g.idr field isLocal g.doc
  |> Guide.setDoc g
...
getIdr field g  =  Core.reach g.idr [field] g.doc 


{-|-}
getIdrs : String -> Guide g a  -> List Identifier 
getIdrs field g = Core.reachMany g.idr [field] g.doc
...
reach fields g  =  Core.reach g.idr fields g.doc 
...
reachMany fields g  =  Core.reachMany g.idr fields g.doc 
...
find objectTest fields g  =  
  let 
    test idr = 
      ...
      |> withDefault False
  in       
    case reachMany fields g
        |> Utility.find test of 
      Nothing -> 
        reach fields g
        |> Maybe.map singleton
